...
public class Tree<T> extends Composite<Div>
  ...
  private class CustomizedTreeGrid<T> extends TreeGrid<T> {
    ...
    private Column<T> setHierarchyColumnWithTitle(ValueProvider<T, ?> valueProvider,
    ...
    public Column<T> setHierarchyColumn(ValueProvider<T, ?> valueProvider,
        ValueProvider<T, VaadinIcon> iconProvider, ValueProvider<T, String> titleProvider) {
      removeAllColumns();
   ...
   *
   * @param valueProvider the item caption provider to use, not <code>null</code>
   */
   ...
   *
   * @param valueProvider the item caption provider to use, not <code>null</code>
   * @param treeData the tree data for component
   */
  ...
  /**
   * Constructs a new Tree Component with given caption and {@code HierarchicalDataProvider}.
   *
   * @param valueProvider the item caption provider to use, not <code>null</code>
   * @param dataProvider the hierarchical data provider for component
   */
   ...
   *
   * @param collection the expanded item
   * @param userOriginated whether the expand was triggered by a user interaction or the server
   */
   ...
   *
   * @param collection the collapsed item
   * @param userOriginated whether the collapse was triggered by a user interaction or the server
   */
   ...
   * <p>
   * If an item is currently expanded, does nothing. If an item does not have any children, does
   * nothing.
   *
   ...
   * <p>
   * {@code depth} describes the maximum distance between a given item and its descendant, meaning
   * that {@code expandRecursively(items, 0)} expands only the given items while {@code
   * expandRecursively(items, 2)} expands the given items as well as their children and
   * grandchildren.
   *
   ...
   *
   * @param items the collection of items to collapse
   */
   ...
   * <p>
   * {@code depth} describes the maximum distance between a given item and its descendant, meaning
   * that {@code collapseRecursively(items, 0)} collapses only the given items while {@code
   * collapseRecursively(items, 2)} collapses the given items as well as their children and
   * grandchildren.
   *
   * @param items the items to expand recursively
   * @param depth the maximum depth of recursion
   ...
   * {@link #setSelectionMode(SelectionMode)}, then this listener is not triggered anymore when
   * selection changes!
   *
   * @param listener the listener to add
   * @return a registration handle to remove the listener
   * @throws UnsupportedOperationException if selection has been disabled with {@link
   * SelectionMode#NONE}
   */
  ...
  /**
   * Use this tree as a multi select in Binder. Throws {@link IllegalStateException} if the tree is
   * not using {@link SelectionMode#MULTI}.
   *
  ...
  /**
   * Use this tree as a single select in Binder. Throws {@link IllegalStateException} if the tree is
   * not using {@link SelectionMode#SINGLE}.
   *
   * @return the single select wrapper that can be used in binder
   */
  ...
  /**
   * Sets the item caption generator that is used to produce the strings shown as the text for each
  ...
  /**
   * Sets the item icon generator that is used to produce custom icons for items. The generator can
   * return <code>null</code> for items with no icon.
   *
   * @param iconProvider the item icon generator to set, not <code>null</code>
   * @throws NullPointerException if {@code itemIconGenerator} is {@code null}
   */
   ...
   * Returning null from the generator results in no custom style name being set.
   *
   * @param classNameGenerator the item style generator to set, not {@code null}
   * @throws NullPointerException if {@code styleGenerator} is {@code null}
   */
  ...
  /**
   * Sets the title generator that is used for generating tooltip descriptions for items.
   *
   * @param titleProvider the item description generator to set, or <code>null</code> to remove a
   * previously set generator
   */
  ...
  /**
   * Adds an item click listener. The listener is called when an item of this {@code Tree} is
   * clicked.
   *
   * @param listener the item click listener, not null
   * @return a registration for the listener
   */
   ...
   * @throws IllegalArgumentException if the provided row is outside the item range
   */
